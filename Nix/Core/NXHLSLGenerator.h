...
public:
    ...
    bool ConvertShaderToHLSL(const std::filesystem::path& shaderPath, const std::string& nslCode, std::string& oHLSLCodeHead, std::string& oHLSLCodeBody);
    void EncodeToGBufferShader(const std::string& strHLSLParam, const std::string& strHLSLBody, std::string& oHLSLFinal);

private:
	std::string ConvertShaderParam(const std::filesystem::path& shaderPath, const std::string& nslCode);
