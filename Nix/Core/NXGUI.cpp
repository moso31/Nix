...
bool NXGUI::Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
...
void NXGUI::RenderDrawData(ImDrawData* draw_data)
{
    // Avoid rendering when minimized
    if (draw_data->DisplaySize.x <= 0.0f || draw_data->DisplaySize.y <= 0.0f)
        return;

    ID3D11DeviceContext* ctx = m_pd3dDeviceContext.Get();
    ...
    if (ctx->Map(m_pVB.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &vtx_resource) != S_OK)
        return;
    if (ctx->Map(m_pIB.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &idx_resource) != S_OK)
        return;
    ...
    ctx->Unmap(m_pVB.Get(), 0);
    ctx->Unmap(m_pIB.Get(), 0);

    // Setup orthographic projection matrix into our constant buffer
    // Our visible imgui space lies from draw_data->DisplayPos (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
    {
        D3D11_MAPPED_SUBRESOURCE mapped_resource;
        if (ctx->Map(m_pVertexConstantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) != S_OK)
            return;
        ...
        ctx->Unmap(m_pVertexConstantBuffer.Get(), 0);
    }
...
void NXGUI::InvalidateDeviceObjects()
{
    if (!m_pd3dDevice)
        return;

    if (m_pFontSampler) { m_pFontSampler->Release(); }
    if (m_pFontTextureView) { m_pFontTextureView->Release(); ImGui::GetIO().Fonts->TexID = NULL; } // We copied m_pFontTextureView to io.Fonts->TexID so let's clear that as well.
    if (m_pIB) { m_pIB->Release(); }
    if (m_pVB) { m_pVB->Release(); }
    ...
    if (m_pVertexConstantBuffer) { m_pVertexConstantBuffer->Release(); }
    if (m_pInputLayout) { m_pInputLayout->Release(); }
    if (m_pVertexShader) { m_pVertexShader->Release(); }
}

bool NXGUI::CreateDeviceObjects()
{
    if (!m_pd3dDevice)
        return false;
    if (m_pFontSampler)
...
void NXGUI::_CreateFontsTexture()
{
    // Build texture atlas
    ImGuiIO& io = ImGui::GetIO();
...
void NXGUI::_SetupRenderState(ImDrawData* draw_data, ID3D11DeviceContext* ctx)
{
    // Setup viewport
    D3D11_VIEWPORT vp;
    ...
    ctx->IASetIndexBuffer(m_pIB.Get(), sizeof(ImDrawIdx) == 2 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT, 0);
    ctx->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    ctx->VSSetShader(m_pVertexShader.Get(), NULL, 0);
    ctx->VSSetConstantBuffers(0, 1, &m_pVertexConstantBuffer);
    ctx->PSSetShader(m_pPixelShader.Get(), NULL, 0);
    ...
    ctx->OMSetBlendState(m_pBlendState.Get(), blend_factor, 0xffffffff);
    ctx->OMSetDepthStencilState(m_pDepthStencilState.Get(), 0);
    ctx->RSSetState(m_pRasterizerState.Get());
}