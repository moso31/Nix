...
public:
    ...
    Ntr<NXTexture2D> CreateTexture2D(const std::string& name, DXGI_FORMAT fmt, UINT width, UINT height, UINT mipLevels = 1, D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE, bool bAutoMakeViews = true);
    ...
    Ntr<NXTexture2DArray> CreateTexture2DArray(const std::string& debugName, DXGI_FORMAT texFormat, UINT width, UINT height, UINT arraySize = 1, UINT mipLevels = 1, D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, bool bAutoMakeViews = true);
