...
void NXCustomMaterial::CompileShader(const std::string& strGBufferShader, const NXMaterialData& guiData, std::string& oErrorMessageVS, std::string& oErrorMessagePS)
{
	...
	if (m_bCompileSuccess)
	{
		...
		if (m_bEnableTerrainGPUInstancing) // for gpu-driven terrain
		{
			...
			ranges2.push_back(NX12Util::CreateDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 2, 1)); // t2, space1 = Splat Map;
			ranges2.push_back(NX12Util::CreateDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 3, 1)); // t3, space1 = Normal Map;
			ranges2.push_back(NX12Util::CreateDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 4, 1)); // t4, space1 = Albedo Map Atlas;
			ranges2.push_back(NX12Util::CreateDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 5, 1)); // t5, space1 = Terrain Albedo Array;
			ranges2.push_back(NX12Util::CreateDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 6, 1)); // t6, space1 = Terrain Normal Array;
...
void NXCustomMaterial::UpdatePSORenderStates(D3D12_GRAPHICS_PIPELINE_STATE_DESC& oPSODesc, const NXMaterialData& guiData)
{
	...
	oPSODesc.BlendState = NXBlendState<>::Create();
	oPSODesc.RasterizerState = NXRasterizerState<D3D12_FILL_MODE_SOLID>::Create();
}
