...
public:
	...
	void GetNearest(const Vector3& position, float& out_distSqr, priority_quque_NXPhoton& out_nearestPhotons, float range = 0.1f);

	void Release();
	
private:
	void Locate(const Vector3& position, const shared_ptr<NXKdTreeNode>& p, float& out_mindistSqr, priority_quque_NXPhoton& out_nearestPhotons);
